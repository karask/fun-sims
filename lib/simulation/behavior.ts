import { Ant, Brood, ColonyState, Debris, Point, Resource, Task, WORLD, CELL, NEST_CENTER, NEST_LAYER_SIZE } from './types';
...
function move(s:ColonyState,a:Ant,dt:number,target:Point|null,ctx:BehaviorContext){
 ...
 const step=speed*dt;
 let p={x:a.x+Math.cos(a.angle)*step,y:a.y+Math.sin(a.angle)*step,z:a.z};
 if(a.view==='nest'){
  if(!waypoint)return;
  const d=distance(a,waypoint),f=Math.min(1,step/(d||1));
  p={x:a.x+(waypoint.x-a.x)*f,y:a.y+(waypoint.y-a.y)*f,z:a.z+((waypoint.z??NEST_CENTER)-a.z)*f};
 }
 if(isOpen(s,p,a.view)){a.x=p.x;a.y=p.y;a.z=p.z;}else{a.angle+=(a.id%2===0?1:-1)*dt*7;if(a.timer>8&&a.view==='surface'&&!a.cargo){a.target=null;a.memory=null;setTask(a,'exploring','A blocked path calls for another route');}}
...
function carryOutside(s:ColonyState,a:Ant,dt:number,ctx:BehaviorContext){
 if(a.view==='nest'){move(s,a,dt,WORLD.nestEntrance,ctx);if(distance(a,WORLD.nestEntrance)<12){a.view='surface';a.z=NEST_CENTER;a.x=WORLD.entrance.x;a.y=WORLD.entrance.y;a.target={...WORLD.midden};}}
...
function decide(s:ColonyState,a:Ant,ctx:BehaviorContext){
 ...
 if(needingCare&&(a.age<35||random(s)<.45)){a.target={x:needingCare.x,y:needingCare.y,z:needingCare.z};a.carryingId=needingCare.id;setTask(a,'nursing','A nearby brood item needs care');return;}
 ...
 if(localBrood.length){const b=localBrood[Math.floor(random(s)*localBrood.length)];a.target={x:b.x,y:b.y,z:b.z};a.carryingId=b.id;setTask(a,'nursing','Tending the next generation');return;}
...
export function updateBehavior(s:ColonyState,a:Ant,dt:number,ctx:BehaviorContext){
 ...
 if(a.cargo==='brood'){
 const b=s.brood.find(b=>b.id===a.carryingId);if(!b){a.cargo=null;a.carryingId=null;return;}move(s,a,dt,a.target,ctx);b.x=a.x;b.y=a.y;b.z=a.z;if(a.target&&distance(a,a.target)<15){b.carriedBy=null;a.carryingId=null;a.cargo=null;a.target=null;setTask(a,'nursing','Brood moved to a more suitable chamber');a.timer=0;}return;
 }
 ...
 if(a.task==='exploring'||a.task==='foraging'){move(s,a,dt,WORLD.nestEntrance,ctx);if(distance(a,WORLD.nestEntrance)<12){a.view='surface';a.z=NEST_CENTER;a.x=WORLD.entrance.x;a.y=WORLD.entrance.y;a.target=a.memory?{...a.memory}:null;a.angle=random(s)*Math.PI*2;setTask(a,'exploring','Emerging from the nest to forage');}return;}
 if(a.task==='nursing'){
 const b=s.brood.find(b=>b.id===a.carryingId&&b.carriedBy===null);if(b){a.target={x:b.x,y:b.y,z:b.z};if(distance(a,b)<14){b.care=clamp(b.care+dt*.08,0,1);const amount=Math.min(s.stores.protein,dt*.009*s.settings.lifecycle);if(b.stage==='larva'){s.stores.protein-=amount;b.nutrition=clamp(b.nutrition+amount*2.5,0,1);}const comfort=broodComfort(s,b);if(comfort<.55&&a.timer>12){const candidates=[{...NURSERY},{...CHAMBERS[2]},{...QUEEN_CHAMBER}].map(p=>nearestOpenNestPoint(s,p));candidates.sort((x,y)=>broodComfort(s,y)-broodComfort(s,x));if(broodComfort(s,candidates[0])>comfort+.04){a.cargo='brood';a.carryingId=b.id;b.carriedBy=a.id;a.target=candidates[0];a.reason='Moving brood toward better moisture and temperature';}}return;}}
 move(s,a,dt,a.target??{...NURSERY},ctx);return;
 }
 if(a.task==='digging'){
 if(!a.target){a.decision=0;return;}const i=cellIndex(a.target);if(s.nest[i]){a.target=null;a.decision=0;return;}
 if(distance(a,a.target)<Math.max(CELL,NEST_LAYER_SIZE)*1.3){s.excavation[i]+=dt*.12*(.4+s.settings.moisture/100);if(s.excavation[i]>=1){s.nest[i]=1;s.excavation[i]=0;s.excavated++;a.cargo='soil';a.target={...WORLD.nestEntrance};setTask(a,'returning','Carrying excavated soil to the surface');}}
