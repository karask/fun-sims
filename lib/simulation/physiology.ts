...
import { clamp, distance } from './spatial';
import { broodComfort, QUEEN_CHAMBER } from './behavior';
...
export function updatePhysiology(s:ColonyState,dt:number){
 ...
 // Stored food can only be accessed from the familiar storage/queen chamber.
 if(a.view==='nest'&&distance(a,QUEEN_CHAMBER)<160&&a.hunger>.15&&s.stores.carbohydrate>0){const food=Math.min(s.stores.carbohydrate,dt*.025*rate,a.hunger*.4);s.stores.carbohydrate-=food;a.hunger=clamp(a.hunger-food*2.5,0,1);a.energy=clamp(a.energy+food,0,1);}
