...
export const settingsSchema=z.object({spiderEncounters:z.boolean(),rivalEncounters:z.boolean(),encounterFrequency:bounded(.25,4),temperature:bounded(5,40),moisture:bounded(5,95),lifecycle:bounded(.25,4),exploration:fraction,pheromoneDecay:bounded(.003,.055),sensitivity:bounded(.2,3),flexibility:bounded(.1,1)}).strict();
const task=z.enum(['exploring','foraging','returning','nursing','digging','cleaning','resting','grooming','defending','retreating','queen']);const kind=z.enum(['carbohydrate','protein','water']);const view=z.enum(['surface','nest']);
...
const enemy=point.extend({id,kind:z.enum(['spider','rival']),angle:finite,mode:z.enum(['roaming','hunting','feeding','returning','retreating']),energy:fraction,age:bounded(0,301),cooldown:bounded(0,10),home:point,memory:point.nullable(),cargo:kind.nullable(),amount:bounded(0,1.601),prey:z.boolean(),reason:z.string().max(250)}).strict();
const encounters=z.object({spiderClock:bounded(0,240),rivalClock:bounded(0,240),arrivals:integer(),repelled:integer(),lostWorkers:integer(),stolenFood:bounded(0,1e9)}).strict();
const stateSchema=z.object({version:z.union([z.literal(1),z.literal(2),z.literal(3)]),simulator:z.literal('ants'),enemies:z.array(enemy).max(4),alarm:z.array(fraction).length(FIELD_COLS*FIELD_ROWS),encounters,seed:integer(4294967295),rng:integer(4294967295),tick:integer(),nextId:id,time:bounded(0,1e9),settings:settingsSchema,ants:z.array(ant).max(3001),brood:z.array(point.extend({id,stage:z.enum(['egg','larva','pupa']),progress:fraction,nutrition:fraction,care:fraction,carriedBy:id.nullable()}).strict()).max(600),resources:z.array(point.extend({id,kind,amount:bounded(0,100000),initial:bounded(.01,100000),radius:bounded(1,80)}).strict()).max(200),obstacles:z.array(point.extend({id,radius:bounded(1,80)}).strict()).max(200),debris:z.array(point.extend({id,view,kind:z.enum(['waste','corpse']),carriedBy:id.nullable()}).strict()).max(10000),nest:z.array(z.union([z.literal(0),z.literal(1)])).min(COLS*ROWS).max(NEST_CELLS),excavation:z.array(bounded(0,1)).min(COLS*ROWS).max(NEST_CELLS),pheromones:z.array(bounded(0,50)).length(FIELD_COLS*FIELD_ROWS),traffic:z.array(bounded(0,1e8)).length(FIELD_COLS*FIELD_ROWS),stores:z.object({carbohydrate:bounded(0,100000),protein:bounded(0,100000),water:bounded(0,100000)}).strict(),queenEggTimer:bounded(0,12),births:integer(),deaths:integer(),excavated:integer(),depositedSoil:integer(),removedWaste:integer(),history:z.array(z.object({time:bounded(0,1e9),population:integer(3000),brood:integer(600),food:bounded(0,200000),foraging:integer(3001)}).strict()).max(600),events:z.array(z.object({time:bounded(0,1e9),text:z.string().max(300)}).strict()).max(30)}).strict();
export function validateSave(input:unknown):ColonyState {
 // Upgrade only recognized legacy versions. Current saves must contain all fields.
 if(input&&typeof input==='object'&&'version' in input&&(input.version===1||input.version===2)){
  const legacy=input as Record<string,unknown>;
  input={...legacy,enemies:[],alarm:Array(FIELD_COLS*FIELD_ROWS).fill(0),encounters:{spiderClock:45,rivalClock:65,arrivals:0,repelled:0,lostWorkers:0,stolenFood:0},settings:{...(legacy.settings as object),spiderEncounters:false,rivalEncounters:false,encounterFrequency:1}};
 }
 ...
 s.version=3;
 const all=[...s.enemies,...s.ants,...s.brood,...s.resources,...s.obstacles,...s.debris];const ids=new Set(all.map(a=>a.id));if(ids.size!==all.length||all.some(a=>a.id>=s.nextId)||!s.nest.some(Boolean)||s.ants.filter(a=>a.task==='queen').length>1||Math.abs(s.time-s.tick/30)>.001)throw new Error('This save contains inconsistent colony data.');
 ...
 if(s.enemies.filter(e=>e.kind==='spider').length>1||s.enemies.filter(e=>e.kind==='rival').length>3||s.enemies.some(e=>(e.home.x>18&&e.home.x<1382&&e.home.y>18&&e.home.y<882)||!isOpen(s as ColonyState,e,'surface',e.kind==='spider'?12:5)||[e,e.home,e.memory].some(p=>p?.z!==undefined&&p.z!==NEST_CENTER)||(e.kind==='spider'&&(e.cargo!==null||e.amount>0))||(e.kind==='rival'&&e.prey)||(e.amount>0)!==(e.cargo!==null)||e.cargo==='water'))throw new Error('This save contains invalid encounter data.');
