...
export type Task = 'exploring' | 'foraging' | 'returning' | 'nursing' | 'digging' | 'cleaning' | 'resting' | 'grooming' | 'queen';
...
export interface Ant extends Point {
  id: number; angle: number; view: View; task: Task; age: number; energy: number; hunger: number;
...
export interface Settings { temperature: number; moisture: number; lifecycle: number; exploration: number; pheromoneDecay: number; sensitivity: number; flexibility: number }
export interface Sample { time: number; population: number; brood: number; food: number; foraging: number }
export interface ColonyState {
  version: 1; simulator: 'ants'; seed: number; rng: number; tick: number; nextId: number; time: number;
...
export type Command =
  ...
  | { type: 'place'; kind: ResourceKind | 'obstacle' | 'erase'; x: number; y: number }
...
export const WORLD = { width: 1400, height: 900, entrance: { x: 680, y: 430 }, nestEntrance: { x: 680, y: 110 }, midden: { x: 845, y: 485 } };
export const CELL = 20, COLS = 70, ROWS = 45, FIELD_CELL = 14, FIELD_COLS = 100, FIELD_ROWS = 65;
export const DEFAULT_SETTINGS: Settings = { temperature: 24, moisture: 65, lifecycle: 1, exploration: 0.45, pheromoneDecay: 0.018, sensitivity: 1, flexibility: 0.6 };
export const TASK_COLORS: Record<Task,string> = { exploring:'#b9c3bf',foraging:'#e2bc79',returning:'#e2bc79',nursing:'#a4aacb',digging:'#c69572',cleaning:'#7bbcb5',resting:'#82948c',grooming:'#8fa88c',queen:'#e4bc77' };
