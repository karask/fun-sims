...
export function useSolarAutosave(engine:SolarEngine,active:boolean,onRestore:()=>void,onError:(message:string)=>void){
 ...
 useEffect(()=>{if(!ready)return;const timer=window.setInterval(()=>{if(latest.current.active&&!document.hidden)void write();},30000);const hidden=()=>{if(document.hidden)void write();};document.addEventListener('visibilitychange',hidden);window.addEventListener('pagehide',write);return()=>{clearInterval(timer);document.removeEventListener('visibilitychange',hidden);window.removeEventListener('pagehide',write);};},[ready,write]);
